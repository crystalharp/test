...
pqueue_push (sweep_line_t *sweep, rectangle_t *rectangle)
{
	...
	if (unlikely (! pqueue_grow (&sweep->pq))) {
	    longjmp (sweep->unwind,
		      CAIRO_STATUS_NO_MEMORY);//_cairo_error (CAIRO_STATUS_NO_MEMORY));
	}
#endif
    LOG_DBG("Error, should not be here!");
    }
					 ...
					 cairo_boxes_t *out)
{
	...
	if (unlikely (rectangles == NULL))
	    return CAIRO_STATUS_NO_MEMORY;//_cairo_error (CAIRO_STATUS_NO_MEMORY);
