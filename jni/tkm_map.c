...
#ifdef TK_BPP_16
    int stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB16_565, w);
    surface =
    cairo_image_surface_create_for_data ((unsigned char *)tk_gdi.pd, CAIRO_FORMAT_RGB16_565, w, h, stride);
#endif
#ifdef TK_BPP_32
...
{
    return (int)floor(TK_ABS(-(lon + 180) / 360 * (1 << GSCALE_FACTOR) * TILE_SIZE_WH));
}
...
unsigned int tk_lat_to_y(double lat)
{
    ...
    y = -(log(y_temp) / (MATH_PI * 2) - 1) * (1 << (GSCALE_FACTOR - 1)) * TILE_SIZE_WH;
...
//    } else {
//        el->left = (x << 8)  + tile_w * (x - ((x >> -bl_dif) << -bl_dif));
//        el->right = el->left + tile_w;
//        el->bottom = (y << 8) +tile_h * (y - ((y >> -bl_dif) << -bl_dif));
//        el->top = el->bottom + tile_h;
//    }
//}
...
int tk_get_tile_buffer(int tile_x, int tile_y, int zoom) {
    struct envelope el;
    int flag = 0;
    int level_dif = 16 - zoom;
    if (zoom < 4 || zoom > 18)
        return -1;
    
    //add for antroid
    clean_tile(zoom); //清除当前所有的tile数据
    tk_engine.bl = get_base_level(zoom);
    set_zlevel(zoom);
    //end
    
    lostdata_idx = 0;
    memset(lostdata, 0, sizeof(struct tk_map_lostdata) * TK_LOST_DATA_PIECE); //清空lostdata
    // 这个envelope实际只用于和region边界求交
    get_tile_box(tile_x, tile_y, tk_engine.bl_dif, &el, TILE_SIZE_WH, TILE_SIZE_WH);
    tk_engine.center_x = (((el.left + el.right) >> 1) << (level_dif + 2)) >> 2;
    tk_engine.center_y = (((el.bottom + el.top) >> 1) << (level_dif + 2)) >> 2;
    tk_reinit_gdicontext(TILE_SIZE_WH, TILE_SIZE_WH);
    calculate_box(TILE_SIZE_WH, TILE_SIZE_WH);
...
double tk_meters_per_pixel(double lat, short int z) {
    if (z <= LEVEL_SKIP)
        z--;
	double meters_per_pixel = MATH_PI * 2 * EARTH_RADIUS / (1 << z) / TILE_SIZE_WH;
...
int tk_get_cid_by_pos_and_level(struct tk_latlon pos, int level)
{
    struct tk_point pos_point;
    int cur_pos_region_id;
    ...
    pos_point.x = tk_lon_to_x(pos.lon);
    pos_point.y = tk_lat_to_y(pos.lat);
    cur_pos_region_id = tk_get_rid_by_point_and_level(&pos_point, level);
...
int tk_get_city_id_by_rid(int rid) {
    int i;
    for (i = 0; i< reg_num; i++) {
        if (reg_list[i].rid == rid)
...
double tk_y_to_lat(int y) {
    double temp = exp((MATH_PI) * 2 * -((double)y / (1 << (16 - 1)) / TILE_SIZE_WH - 1));
    double lat = asin((temp - 1) / (temp + 1));
    return lat * 180 / MATH_PI;
}

double tk_x_to_lon(int x) {
    double lon = ((double)x / (TILE_SIZE_WH * (1 << 16))) * 360;
    return lon - 180;
}
