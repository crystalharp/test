...
struct region {
    ...
    int code_con_num;//huffman编码配置区域个数
    int map_data_bias;
    int level_num;//huffman索引个数
...
static int load_reg(int rid)
{
    ...
    hfcoder_length = GETNUM3B(buff);
    index_length = GETNUM3B(buff + 3);
    
    reg[idx].map_data_bias = hfcoder_length + index_length + 6;
    hfcode_block = malloc(hfcoder_length);
    fread(hfcode_block, 1, hfcoder_length, reg[idx].fp);
    
    /* get the version number */
    memcpy(reg[idx].version, hfcode_block, 6);
    reg[idx].index = malloc(index_length);
    fread(reg[idx].index, 1, index_length, reg[idx].fp);
    reg[idx].index_count = index_length / 6;
    reg[idx].level_num = (hfcode_block[TK_VERNO_LENGTH+0] << 8) + hfcode_block[TK_VERNO_LENGTH + 1];
    reg[idx].code_levels = malloc(reg[idx].level_num * TK_BLEVEL_NUM
                                  * TK_CODER_UNIT_SIZE);
    reg[idx].code_con_num = hfcode_block[TK_VERNO_LENGTH + 2];
    reg[idx].code_read_con = malloc(TK_STOP_UNIT_SIZE * reg[idx].code_con_num);
    
    memcpy(reg[idx].code_read_con, hfcode_block + TK_VERNO_LENGTH + 3, reg[idx].code_con_num
           * TK_STOP_UNIT_SIZE);
    memcpy(reg[idx].code_levels, hfcode_block + TK_VERNO_LENGTH + 3 + reg[idx].code_con_num
           * TK_STOP_UNIT_SIZE, reg[idx].level_num * TK_BLEVEL_NUM * TK_CODER_UNIT_SIZE);
    ...
    for (i = 0; i < TK_BLEVEL_NUM && res < reg[idx].index_count; i++) {
...
static void release_reg(int idx)
{
    ...
    if (reg[idx].code_levels) {
        free(reg[idx].code_levels);
    }
    if (reg[idx].code_read_con) {
        free(reg[idx].code_read_con);
    }
    if (reg[idx].verifycode) {
        free(reg[idx].verifycode);
        reg[idx].verifycode = NULL;
    }
    if (reg[idx].cached_polygon) {
...
static int load_stylecfg(char *fname, struct styles_buffer *buff)
{
    ...
    buff->priority = malloc(sizeof(short int) * buff->layer_num);
    memset(buff->priority, -1, sizeof(short int) * buff->layer_num);
    ...
    for (i = 0; i < buff->layer_num; i++) {
        ...
        --setting_num;
        priority = get_nInt(str, &offset);
        if ((priority < buff->layer_num)) {
            buff->priority[priority] = i;
        };
        --setting_num;
        if (setting_num > 0) {
            int cir = 0;
...
int update_swcolor(int rid, gdi_color color,char* name) {
    int i = 0;
...
int load_sw_colorcfg()
{
    ...
    *(str_buff + fsize) = '\0';
    ...
    while (sgets(line, TK_MAX_LINE_LENGTH, str_buff) != NULL) {
        ...
        update_swcolor(cityid, TK_COLOR2INDEX(strtol(color,&end,16)), sw_name);
        memset(city,0,sizeof(city));
        memset(sw_name,0,sizeof(sw_name));
...
static int ref_tile_x = 0;//城市左下角tile参考坐标
...
/*
 * 寻找tile(x,y), 并将数据读到tk_tile_databuf中, 其实读取操作可以不在这里做(cm)
 */
static int seek_tile(int x, int y, short int* state, int region_id, int *tile_length)
{
    ...
    *state = TK_READ_OK;
    ...
    index_size = reg[cur_idx].index_count;
...
static short int read_feature_type(int rid)
{
    unsigned short translated_code;
    unsigned char *levels;
    
    levels = reg[cur_idx].code_levels;
    translated_code = read_code_idx();//得到huffman索引值
...
static void bit_read_string(char *res, unsigned int length) {
    unsigned int i;
    if (length == 0) {
        return;
    }
...
static int read_point(struct tk_point *res, int must)
{
    ...
    if (i != -1) {
        ...
        unsigned int x, y;
        x = read_tile_databuf((unsigned char)(TILE_SIZE_BITS + cur_tile_level));//这是什么？
...
static int read_feature(int rid, int* size, struct feature *this, int point_tile)
{
    int i;
    int need_read;
    int point_num = 0;
    ...
    feature_type = read_feature_type(rid);
    if (feature_type != TK_CODE_NOT_DISPLAY) {
        if (feature_type >= tk_engine.ps->layer_num) {
            feature_type = tk_engine.ps->layer_num - 1;
        }
        need_read = 1;
        this->name_index = -1;
        this->name_length = -1;
        ...
        if (read_tile_databuf(1) == 1) {//=1表示有名字
            this->name_length = (short)read_tile_databuf(8);
            if (this->name_length < 0) {
                return TK_READ_FT_NAMELEN_ERR;
            }
            this->name = cur_mem_pool->tk_map_names + cur_mem_pool->cur_name_index;
            bit_read_string(this->name, this->name_length);
            cur_mem_pool->cur_name_index += this->name_length;
...
static int read_tile(int x, int y, short *state, int region_id, struct tile **tl_res, int level_dif)
{
    struct tile *res;
    int size;
    int len;
    ...
    if (point_num < 0 || point_num > 10000) {
        if (tk_tile_databuf != NULL) {
            free(tk_tile_databuf);
            tk_tile_databuf = NULL;
        }
    ...
    size = 0;
    while (size < len) {
        int ft_flag = read_feature(region_id, &size, res->features + (res->fnum), point_num);
        //printf("flag == %d\n", ft_flag);
        if (ft_flag > 0) {
            if (tk_tile_databuf != NULL) {
                free(tk_tile_databuf);
    ...
    res->coder_lat = y - ((y - ref_tile_y) % (1 << cur_tile_level));//基本坐标
    ...
    res->level_dif = level_dif;
    res->bias_x = (res->coder_lon) << (8 - level_dif); //相对基本像素坐标
    res->bias_y = (res->coder_lat) << (8 - level_dif);
    
    res->bbox.bottom = res->coder_lat;//基本坐标,只用来判断两个tile相邻
    ...
    while (temp) {
        if ((((x - temp->coder_lon) >> temp->level) == 0)
            ...
            if (i >= tk_engine.tile_num) {//else then the tile is just added
                if (i > 0) {// when i == 0, need not to change order
            ...
            for (x = bbox->left - 1 * add_tile_bound; x <= bbox->right + 1 * add_tile_bound; x++) {
                for (y = bbox->bottom - 1 * add_tile_bound; y <= bbox->top + 1 * add_tile_bound; y++) {
                    ...
                    if (index < 0) {
                        ...
                        flag = read_tile(x, y, &state, reg_in_bound[cir_id], &tempt, level_dif);
                        if (flag == TK_READ_OK) {
                            if (tempt != NULL) {
                                tempt->is_active = 1;
...
{
    bbox->bottom = (tk_engine.cur_bbox.bottom) >> (TILE_SIZE_BITS + TK_BASE_LEVEL_A - tk_engine.bl);// tile坐标（相对基本坐标）
...
void calculate_box(int w, int h)
{
	...
	tk_gdi.rtview.left = -8;
    tk_gdi.rtview.right = w + 8;
    tk_gdi.rtview.top = -8;
    tk_gdi.rtview.bottom = h + 8;
    
    tk_gdi.feature_clip_view.left = -w / 16;
    tk_gdi.feature_clip_view.right = w + w / 16;
    tk_gdi.feature_clip_view.top = -h / 16;
    tk_gdi.feature_clip_view.bottom = h + h / 16;
    
    /* calculate the four corners' global coordination */
    zoom = 16 - tk_engine.current_z;
    tk_engine.cur_bbox.left = tk_engine.center_x - ((w >> 1) << (zoom + 2) >> 2);//乘上256之后的坐标, 即最细级别mercator坐标
    tk_engine.cur_bbox.right = tk_engine.center_x + ((w >> 1) << (zoom + 2) >> 2);
    tk_engine.cur_bbox.top = tk_engine.center_y + ((h >> 1) << (zoom + 2) >> 2);
    tk_engine.cur_bbox.bottom = tk_engine.center_y - ((h >> 1) << (zoom + 2) >> 2);
}
            ...
            if (TK_ENV_NO_OVERLAP((*tile_box), reg_bounds[i].env) == 0) {
                ...
                if (is_inpoly) { //中心点被region包含
                    if (regnum_in_bound < TK_REGION_CACHED_MAX){
                        ...
                        reg_in_bound[regnum_in_bound-1] = i;//这是为何？放不下还要放在最后一个？
                    }
...
static void _tile_syc_map(struct tile* current_tile)
{
    ...
    for (j = 0; j < current_tile->fnum; j++) {
        ...
        if (cur_feature->type > tk_engine.c_styles.layer_num && cur_feature->type > tk_engine.n_styles.layer_num) {
            continue;
        }
...
int load_tile(int x, int y, struct envelope *tile_box) {
    ...
    get_tile_level_16_bound(tile_box, &el);// 获取24级下mercator坐标放在el里，tile_box只用到这里
...
void tk_update_buffer_screens(struct envelope clip_scr, struct envelope draw_scr)
{
    ...
    while (cur_tl) {
        if (cur_tl->flag == tk_engine.tile_flag) {
            lvl_dif = cur_tl->level_dif;
            if (tk_engine.current_z <= 16) {
                tile_bias_x = (short)(((cur_tl->coder_lon) << (8 - lvl_dif))
                                      - (tk_engine.cur_bbox.left >> (16 - tk_engine.current_z)));// 相对基本像素坐标
                //当前比例尺下参考点的偏差
                tile_bias_y = (short)(((cur_tl->coder_lat) << (8-lvl_dif))
                                      - (tk_engine.cur_bbox.bottom >> (16 - tk_engine.current_z)));
                //转换为16级坐标下的屏幕坐标
                cur_tl->whole_screen.left = (draw_scr.left - tile_bias_x) << lvl_dif; //基本像素坐标，屏幕坐标是相对基本像素坐标
...
static void draw_map(struct envelope filter, int is_file_not_found_scene)
{
    ...
    tk_update_buffer_screens(tk_gdi.feature_clip_view, vm_screen);
    ...
    if ( tk_engine.current_z >= 10) {
        for(i = 0; i < tk_engine.ps->layer_num - 3; i++){
            layindex = tk_engine.ps->priority[i];
            if (layindex < 0) {
                continue;
            }
            pl = tk_engine.layer_list + layindex;
            pstyle = tk_engine.ps->styles + layindex;
            if (pl == NULL)
                continue;
...
void reset_tile_screen(struct envelope *env) {
	env->left = 0;
	env->right = 255;
	env->top = 0;
	env->bottom = 255;
...
int tk_get_rid_by_point(struct tk_point *pt)
{
    ...
    return tk_get_rid_by_point_and_level(pt, level);
