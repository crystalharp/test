...
int color_bg[]={15460060, 11915932, 10072784, 16706462, 16570228, 16695877};
int find_color(int color)
{
    int i = 0;
    for(; i < sizeof(color_bg); i++){
        if(color_bg[i] == color)
            return i;
    }
    return -1;
                              ...
                              int32_t                  height)
{
	...
	while (w--)
	{
        if(((x <= tk_gdi.undo_view.right ) && (x >= tk_gdi.undo_view.left )) && \
            ((y <= tk_gdi.undo_view.bottom ) && (y >= tk_gdi.undo_view.top ))){
            //*dst = 0x000000;
            dst++;
            x++;
            mask++;
...
fast_composite_scaled_nearest (pixman_implementation_t *imp,
			       ...
			       int32_t                  height)
{
    printf("insert into fast_composite_scaled\n");
    uint32_t       *dst_line;
    uint32_t       *src_line;
    int             dst_stride, src_stride;
               ...
               uint32_t  xor)
{
    ...
    dest_x = x;
	for (i = 0; i < width; ++i){
        if(((dest_x <= tk_gdi.undo_view.right  ) && (dest_x >= tk_gdi.undo_view.left )) && \
            ((dest_y <= tk_gdi.undo_view.bottom ) && (dest_y >= tk_gdi.undo_view.top ))){
            //dest_x++;
            //continue;
        }
	    bits[i] = xor;
