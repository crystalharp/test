           ...
           int dir)
{
    ...
    edge->line.p1 = *p1;//为何不直接按top和bottom截断？
    edge->line.p2 = *p2;//p1,p2用来算extents?
    edge->top = top;
    edge->bottom = bottom;
    edge->dir = dir;
    ...
    if (p1->x < polygon->extents.p1.x || p1->x > polygon->extents.p2.x) {
        ...
        if (top != p1->y)
            x = _cairo_edge_compute_intersection_x_for_y (p1, p2, top);//为什么在这里才截断，而不是算好了直接传进来
