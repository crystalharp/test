                   ...
                   cairo_antialias_t			 antialias)
{
    ...
    if (extents->is_bounded)
        needs_clip = extents->clip->path != NULL;//一般情况下polygon已经clip过了
    else
                                ...
                                cairo_antialias_t		 antialias)
{
    ...
    if (status == CAIRO_INT_STATUS_UNSUPPORTED) {
        ...
        polygon.num_limits = 0;//只limit，不算limits[i]
        ...
        if (likely (status == CAIRO_INT_STATUS_SUCCESS)) {
            ...
            if (extents->is_bounded) {
                extents->clip = _cairo_clip_copy_path (extents->clip);//复制路径
                extents->clip = _cairo_clip_intersect_box(extents->clip,
                                                          &polygon.extents);//让clip里的box按polygon.extents调整
            }
