            ...
            int clockwise)
{
    ...
    if (clockwise) {//顺时针时，外圈按顺时针的点来，否则用逆时针的点
        inpt = &in->cw;
        outpt = &out->cw;
        outer = &stroker->cw;
    } else {
    ...
    switch (stroker->style.line_join) {
        ...
        case CAIRO_LINE_JOIN_MITER:
        default: {
            /* dot product of incoming slope vector with outgoing slope vector 单位向量点乘用来表示cos夹角*/
            double	in_dot_out = in->dev_slope.x * out->dev_slope.x + in->dev_slope.y * out->dev_slope.y;
            double	ml = stroker->style.miter_limit;
            
            /* Check the miter limit -- lines meeting at an acute angle
             * can generate long miters, the limit converts them to bevel(如果角度太小，会使得斜接线太长，若超过了斜接的限度，要转成斜切角)
             *
             * Consider the miter join formed when two line segments
             * meet at an angle psi:
             *
             ...
             *
             */
            if (2 <= ml * ml * (1 + in_dot_out)) {//(如果角度太小，会使得斜接线太长，若超过了斜接的限度，要转成斜切角方式,也就是直接break)
                double		x1, y1, x2, y2;
                double		mx, my;
                double		dx1, dx2, dy1, dy2;
                double		ix, iy;
                 ...
                 * faces by comparing the slopes
                 */
                //当两线段接近平行时，有可能交点会在两线段同一侧？无论如何这里保证交点在两线段之间没问题。
                if (slope_compare_sgn (fdx1, fdy1, mdx, mdy) !=
                    slope_compare_sgn (fdx2, fdy2, mdx, mdy))
                {
                    cairo_point_t p;
...
//只有这里的_cairo_polygon_add_contour把轮廓线加到多边形中
static void
         ...
         const cairo_point_t *point)
{
    ...
    //注意这里是为前一条路径添加两端线头，而不是为前一条路径添加结尾线头并为当前新路径添加开端线头
    add_caps (stroker);
...
#endif
    //虽然has_current_face＝false, 但current_face.point还是要设置，以后line_to要用到
         ...
         const cairo_point_t *point)
{
    ...
    //这时has_initial_sub_path为true而has_first_face和has_current_face为false
