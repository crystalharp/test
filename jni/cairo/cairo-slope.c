 ...
 */
//注意这里y轴向下，因此跟普通的y轴向上坐标系相反
int
_cairo_slope_compare (const cairo_slope_t *a, const cairo_slope_t *b)
{
    cairo_int64_t ady_bdx = _cairo_int32x32_64_mul (a->dy, b->dx);//从b到a旋转叉乘ya*xb - yb*xa
    cairo_int64_t bdy_adx = _cairo_int32x32_64_mul (b->dy, a->dx);//<0，a到b逆时针。>0，a到b顺时针。
    int cmp;
    //从b旋转到a，逆时针返回正，顺时针返回负？
