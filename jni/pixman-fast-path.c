                              ...
                              int32_t                  height)
{
	...
	while (w--)
	{
        if(((x <= tk_gdi.unuse_view.right ) && (x >= tk_gdi.unuse_view.left )) && \
            ((y <= tk_gdi.unuse_view.bottom ) && (y >= tk_gdi.unuse_view.top ))){
            //*dst = 0x000000;
            dst++;
            x++;
            mask++;
               ...
               uint32_t  xor)
{
    ...
    while (height--)
    {
        dest_x = x;
	    for (i = 0; i < width; ++i){
            if(dest_x <= tk_gdi.unuse_view.right && dest_x >= tk_gdi.unuse_view.left ){ 
                if(dest_y <= tk_gdi.unuse_view.bottom && dest_y >= tk_gdi.unuse_view.top ){
                dest_x++;
                continue;
                }
            }   
