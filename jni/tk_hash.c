...
#define DEAD_ENTRY ((tk_hash_entry_t *) 0x1)
...
static const unsigned long tk_hash_table_sizes[] = {
    43,
    73,
    151,
    283,
...
struct _tk_hash_table {
    tk_hash_keys_equal_func_t keys_equal;
    
    tk_hash_entry_t *cache[32];
    
    const unsigned long *table_size;
    tk_hash_entry_t **entries;
...
/**
 * _tk_hash_table_uid_keys_equal:
 * @key_a: the first key to be compared
 * @key_b: the second key to be compared
 *
 * Provides a #tk_hash_keys_equal_func_t which always returns
 ...
 **/
static tk_bool_t
_tk_hash_table_uid_keys_equal (const void *key_a, const void *key_b)
{
    return TK_YES;
}

/**
 * _tk_hash_table_create:
...
_tk_hash_table_create (tk_hash_keys_equal_func_t keys_equal)
{
    ...
    hash_table = malloc (sizeof (tk_hash_table_t));
    ...
    if (keys_equal == NULL)
        hash_table->keys_equal = _tk_hash_table_uid_keys_equal;
    else
        hash_table->keys_equal = keys_equal;
    
    memset (&hash_table->cache, 0, sizeof (hash_table->cache));
    hash_table->table_size = &tk_hash_table_sizes[0];
    
    hash_table->entries = calloc (*hash_table->table_size,
                                  sizeof (tk_hash_entry_t *));
 ...
 * _tk_hash_table_destroy is called. It is a fatal error otherwise,
...
static tk_hash_entry_t **
_tk_hash_table_lookup_unique_key (tk_hash_table_t *hash_table,
                                     tk_hash_entry_t *key)
{
 ...
 * termination points.
 *
 * Return value: %TK_STATUS_SUCCESS if successful or
 * %TK_STATUS_NO_MEMORY if out of memory.
 **/
static tk_status_t
_tk_hash_table_manage (tk_hash_table_t *hash_table)
{
    tk_hash_table_t tmp;
    ...
    if (hash_table->live_entries > live_high)
    {
        ...
        assert (tmp.table_size - tk_hash_table_sizes <
                ARRAY_LENGTH (tk_hash_table_sizes));
    }
    else if (hash_table->live_entries < live_low)
    {
        /* Can't shrink if we're at the smallest size */
        if (hash_table->table_size == &tk_hash_table_sizes[0])
    ...
    tmp.entries = calloc (new_size, sizeof (tk_hash_entry_t*));
    if (unlikely (tmp.entries == NULL))
        return TK_STATUS_NO_MEMORY;
    
    for (i = 0; i < *hash_table->table_size; ++i) {
        if (ENTRY_IS_LIVE (hash_table->entries[i])) {
            *_tk_hash_table_lookup_unique_key (&tmp, hash_table->entries[i])
...
/**
 * _tk_hash_table_lookup:
 * @hash_table: a hash table
 * @key: the key of interest
 *
 * Performs a lookup in @hash_table looking for an entry which has a
 * key that matches @key, (as determined by the keys_equal() function
 * passed to _tk_hash_table_create).
 *
 ...
 * the entry obtained with _tk_hash_table_lookup. Or if absolutely
 * necessary, use _tk_hash_table_remove first.
 *
 * Return value: %TK_STATUS_SUCCESS if successful or
 * %TK_STATUS_NO_MEMORY if insufficient memory is available.
 **/
tk_status_t
_tk_hash_table_insert (tk_hash_table_t *hash_table,
                          tk_hash_entry_t *key_and_value)
{
    tk_hash_entry_t **entry;
    tk_status_t status;
    ...
    status = _tk_hash_table_manage (hash_table);
    if (unlikely (status))
        return status;
    
    entry = _tk_hash_table_lookup_unique_key (hash_table, key_and_value);
...
static tk_hash_entry_t **
_tk_hash_table_lookup_exact_key (tk_hash_table_t *hash_table,
                                    tk_hash_entry_t *key)
{
    unsigned long table_size, i, idx, step;
    tk_hash_entry_t **entry;
...
_tk_hash_table_remove (tk_hash_table_t *hash_table,
                          tk_hash_entry_t *key)
{
    *_tk_hash_table_lookup_exact_key (hash_table, key) = DEAD_ENTRY;
    hash_table->live_entries--;
    ...
    if (hash_table->iterating == 0) {
        ...
        _tk_hash_table_manage (hash_table);
    }
}

/**
 * _tk_hash_table_foreach:
 * @hash_table: a hash table
