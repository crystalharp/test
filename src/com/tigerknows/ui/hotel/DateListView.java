...
public class DateListView extends LinearLayout implements View.OnClickListener {
    ...
    void refreshTitle() {
        titleTxv.setText(getCheckDescription(false));
    }
    
    public SpannableStringBuilder getCheckDescription(boolean confirm) {
        today.add(Calendar.DAY_OF_YEAR, (confirm ? confirmCheckinPosition : checkinPosition)-2);
        StringBuilder s = new StringBuilder();
        s.append(monthDayFormat.format(today.getTime())+context.getString(R.string.hotel_checkin_));
        int indexDay = s.length();
        s.append(((confirm ? confirmCheckoutPosition : checkoutPosition)-1)+1);
        int indexN = s.length();
