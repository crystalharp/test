...
//    private MapTextThread mapTextThread;
	...
	private LinkedHashMap<Tile, TileInfo> tileInfos = new LinkedHashMap<Tile, TileInfo>(max_tile_texture_ref * 2, 0.75f, true) {
	...
	private TexturePool texturePool = new TexturePool(max_tile_texture_ref + CONFIG.TILE_THREAD_COUNT);
...
//        MapLayerProperty.getInstance(MapLayerType.SATELLITE).templateSeedTileUrl=
//        	"http://www.globexplorer.com/tiles/decarta?key="+CONFIG.SATELLITE_KEY+"&LL=37.786505,-122.39862&ZOOM=9&CACHEABLE=true&WIDTH="+CONFIG.TILE_SIZE+"&HEIGHT="+CONFIG.TILE_SIZE+"&FORMAT=PNG&N=0&E=0";
//    	       
//        try{
//        	tileCache.init(context);
//        }catch(APIException e){
//        	LogWrapper.e("TilesView", "tileCache.init exception:"+e.getMessage());
//        	e.printStackTrace();
//        }
//        tileThreadPool.startAllThreads();
...
// tigerknows			if(!MapLayerProperty.getInstance(MapLayerType.STREET).sessionId.equals(""+CONFIG.STATELESS_SESSION_ID))
...
// tigerknows add end
        		if(infoWindow.isVisible() && infoWindow.getMercXY()!=null) {
            		...
            		if(snapToInfoWindow(event.getX(0),event.getY(0),relativeXY)){
        				infoWindowClicked=true;
        				event.setLocation(relativeXY.x-InfoWindow.INFO_BORDER_SIZE, relativeXY.y-InfoWindow.INFO_BORDER_SIZE);
    		...
    		if(!multiTouch && pCount==1){
    			...
    			if(infoWindowClicked){
    				if(snapToInfoWindow(event.getX(0),event.getY(0),null)){
    	    			...
    	    			infoWindow.setBackgroundColor(InfoWindow.BACKGROUND_COLOR_CLICKED);
    	    			if(oriColor!=InfoWindow.BACKGROUND_COLOR_CLICKED) refreshMap();
    	    		}else{
    	    			int oriColor=infoWindow.getBackgroundColor();
        		...
        		if(maxMoveFromTouchDown>SAME_POINT_MOVED_DISTANCE_MAX*density*SAME_POINT_MOVED_DISTANCE_MAX*density){
        			resetLongTouchTimer();
        		}
...
//        		}
        		...
        		if(Math.abs(event.getX(0)-lastTouch.x)+Math.abs(event.getY(0)-lastTouch.y)>ABNORMAL_DRAGGING_DIST*density){
    			...
    			if(touchMode==0){//zooming
        			...
        			if(distConv<MIN_PINCH_DISTANCE*density) {
						...
						if((centerDistXConv+centerDistYConv)>ABNORMAL_PINCH_CENTER_DIST*density){
							lastDistConv=0;
...
// tigerknows delete end
    				...
    				if(lastTouchY==null){
        				...
        				if(delY0*delY1>0){
        					synchronized(drawingLock){
        						float delX=(delY0+delY1)/2*MapView.MAP_TILT_MIN/(XROTATION_YDIST*density);
            					rotateX(delX);
            				}
        					refreshMap();
        				}
        	...
        	if(!multiTouch && pCount==1){
    			...
    			if(lastTouchDownTime!=0 && (touchUpTime-lastTouchDownTime)<CLICK_DOWN_UP_TIME_MAX
            			&& (Math.abs(event.getX(0)-lastTouchDown.x)+Math.abs(event.getY(0)-lastTouchDown.y))<SAME_POINT_MOVED_DISTANCE_MAX*density){
    			...
    			if(lastTouchDownTime!=0 && (touchUpTime-lastTouchDownTime)<CLICK_DOWN_UP_TIME_MAX
        			...
        			if(cluster!=null){
        				...
        				if(lastTouchUpTime!=0 && (touchUpTime-lastTouchUpTime)<DOUBLE_CLICK_INTERVAL_TIME_MAX
        						&& (Math.abs(event.getX(0)-lastTouchUp.x)+Math.abs(event.getY(0)-lastTouchUp.y))<SAME_POINT_MOVED_DISTANCE_MAX*density){
        					lastTouchUp.x=0;
        					lastTouchUp.y=0;
        					lastTouchUpTime=0;
     ...
     * @param position
     */
	public void panToPosition(Position position, long duration, MapView.MoveEndEventListener listener) throws APIException{
...
//	}
    private void adjustCenter(){
		...
		if(Math.abs(centerDelta.x)>thresh || Math.abs(centerDelta.y)>thresh){
...
//            }
    ...
    public void zoomView(float newZoomLevel, XYFloat zoomCenterXY) throws APIException{
    	...
    	if(newZoomLevel < CONFIG.ZOOM_LOWER_BOUND || newZoomLevel > CONFIG.ZOOM_UPPER_BOUND){
    		LogWrapper.d("TilesView","zoomTo invalid zoom level:"+newZoomLevel+", must between "+CONFIG.ZOOM_LOWER_BOUND+"-"+CONFIG.ZOOM_UPPER_BOUND);
    		return;
    	}
    	try{
        	zooming = true;
...
//        		clearTilesWaitForLoading();
	...
	private void configureMapLayer(){
		...
		for(int i=0;i<mapLayers.size();i++){
			...
			if(MapView.MapType_MapLayer_Visibility.get(mapType).contains(mapLayerProperty.mapLayerType)){
				...
				if(mapLayerProperty.tileImageSizeFactor>sizeFactor) sizeFactor=mapLayerProperty.tileImageSizeFactor;
				visibleLayerNum++;
			}else{
		...
		LogWrapper.i("TilesView","configureMapLayer visible num,max image,image size,max texture ref:"+visibleLayerNum+","+tileInfos.size()+","+max_tile_texture_ref);
	}
	...
	private boolean snapToInfoWindow(float screenX,float screenY, XYFloat relativeXY){
		float scaleToM=mapMode.middleZ/(float)mapMode.nearZ;
		...
		synchronized (longTouchLock) {
			...
			longTouchTimer.schedule(new TimerTask() {
				...
				public void run() {
					synchronized(longTouchLock){
						longClicked=true;
						Position pos=TilesView.this.screenXYConvToPos(xy0Conv.x, xy0Conv.y);
						TilesView.this.mParentMapView.executeLongClickListeners(pos);
						resetLongTouchTimer();
					}
				}
    ...
    public class MapRender implements GLSurfaceView.Renderer {
		...
		private boolean isLabelShown(Label label) {
			...
			if(shownlabelList == null || shownlabelList.size() == 0 || (label.pointNum == 1 && label.type != 0 && label.type != 1)) {
				return false;
			}
			for(int i = 0, shownNum = shownlabelList.size(); i < shownNum; ++ i) {
				Label shownLabel = shownlabelList.get(i);
				if(shownLabel.type == label.type && shownLabel.name.equals(label.name)) {
					return true;
				}
			}
			return false;
		...
		private boolean shownLabels(float zoomScale, boolean isGetAll, boolean isDrawNew) {
			int i, j, labelNum, priorityNum = maxLabelPriority;
			boolean isFading = false;
			Label.IntegerRef maxLabelToDraw = new Label.IntegerRef(MAX_LABEL_DRAW_PER_FRAME);
			labelGrid.clean();
			for(i = 0; i < priorityNum; ++i) {
				...
				for(j = 0; j < labelNum; ++j) {
					...
					label.draw(new XYInteger(displaySize.x/2, displaySize.y/2), 
							centerXYZ, centerDelta, mapMode.getzRotation(), 
							mapMode.getSinZ(), mapMode.getCosZ(), zoomScale, labelGrid, TEXTURE_COORDS, mVertexBuffer, isDrawNew, maxLabelToDraw);
		            ...
		            if (!isFading && (label.state == Label.LABEL_STATE_FADE_IN || label.state == Label.LABEL_STATE_FADE_OUT || label.state == Label.LABEL_STATE_WAITING)) {
		                isFading = true;
		            }
				}
			}
				...
				for (j = 0; j < labelNum; ++j) {
					...
					if (label.state == Label.LABEL_STATE_PRIMITIVE) {
						...
						label.draw(new XYInteger(displaySize.x / 2,
								...
								TEXTURE_COORDS, mVertexBuffer, isDrawNew, maxLabelToDraw);
						if (label.state != Label.LABEL_STATE_CANT_BE_SHOWN) {
							shownLabelList.add(label);
						} else {
							if (label.state == Label.LABEL_STATE_CANT_BE_SHOWN)
		...
		public MapRender(Context context) {
	        ...
	        labelGrid = new Grid(displaySize.x, displaySize.y, 4 + (int)(2 * Globals.g_metrics.scaledDensity));
		}
		...
		private void clearTiles(GL10 gl) {
				...
				try{
                	...
                	for (int s = 0; s < mapLayers.size(); s++) {
	                    ...
	                    if(bottomDist>=topDist && rightDist>=leftDist){
	                    	...
	                    	for (int i = topDist-1; i <= bottomDist+1; i++) { // -4,+4 暴力：为了避免绘制线路或气泡在屏幕边缘时不能被判断为屏幕之内的问题
	                    		...
	                    		for (int j = leftDist-1; j <= rightDist+1; j++) { // -4,+4暴力：为了避免绘制线路或气泡在屏幕边缘时不能被判断为屏幕之内的问题
	                    			...
	                    			TileInfo tileInfo = tileInfos.get(requestTile);
	                    			...
	                    			if(tileInfo != null) {
	                    				...
	                    				if(tileInfo.tileTextureRef != 0) {
	                    					...
	                    					textureRef = texturePool.getAvailabelTexture();
	                    					if(textureRef != 0) {
	                    						try {
	                    							textureRef = tileInfo.copyToTexture(gl, textureRef);
	                    						} catch (Exception e) {
	                    							texturePool.removeTexture(textureRef);
	                    							textureRef = 0;
	                    						}
	                    					}
	                    					else {
	                    						textureRef = tileInfo.generateTileTextureRef(gl);
	                    						if(textureRef != 0)
	                    							texturePool.putTexture(textureRef);
	                    					}
                    						if(textureRef == 0) {
	                    	...
	                    	mapLayer.mainLayerDrawPercent=(float)willDrawTiles.size()/((bottomDistO-topDistO+1)*(rightDistO-leftDistO+1));
	                    ...
	                    if (mapLayer.centerXY != null && (mapLayer.mainLayerDrawPercent<1.0f || drawStart-fadingStartTime<CONFIG.FADING_TIME*1E6)) {
	                        ...
	                        double mapScaleZL = Math.pow(2, mapLayer.centerXYZ.z - centerXYZ.z);
	                        xxZL = mapLayer.centerXY.x - centerXY.x*mapScaleZL;
		                    yyZL = -(mapLayer.centerXY.y - centerXY.y*mapScaleZL);
		                    ...
		                    double zoomScaleZL=Math.pow(2,zoomLevel-mapLayer.centerXYZ.z);
		                    int leftDistOZL,rightDistOZL,topDistOZL,bottomDistOZL;
		                    int leftDistZL=leftDistOZL=(int)(Math.floor((-(xxZL+mapLayer.centerDelta.x)*zoomScaleZL-displaySizeXL)/(CONFIG.TILE_SIZE*zoomScaleZL)+0.5));
		                	if(leftDistZL<-gridSizeXL) leftDistZL=-gridSizeXL;
		                	int rightDistZL=rightDistOZL=(int)(Math.ceil((-(xxZL+mapLayer.centerDelta.x)*zoomScaleZL+displaySizeXR)/(CONFIG.TILE_SIZE*zoomScaleZL)-0.5));
		                	if(rightDistZL>gridSizeXR) rightDistZL=gridSizeXR;
		                	int topDistZL=topDistOZL=(int)(Math.floor((-(yyZL+mapLayer.centerDelta.y)*zoomScaleZL-displaySizeYT)/(CONFIG.TILE_SIZE*zoomScaleZL)+0.5));
		                	if(topDistZL<-gridSizeYT) topDistZL=-gridSizeYT;
		                	int bottomDistZL=bottomDistOZL=(int)(Math.ceil((-(yyZL+mapLayer.centerDelta.y)*zoomScaleZL+displaySizeYB)/(CONFIG.TILE_SIZE*zoomScaleZL)-0.5));
		                	...
		                	if(bottomDistZL>=topDistZL && rightDistZL>=leftDistZL){
		                		gl.glPushMatrix();
			                	gl.glTranslatef(displaySize.x/2f,displaySize.y/2f,0);
			                	gl.glScalef((float)zoomScaleZL, (float)zoomScaleZL, 1);
			                	gl.glTranslatef(-displaySize.x/2f+(float)xxZL,-displaySize.y/2f+(float)yyZL,0);
			                	int drawNum = 0;
	                            for (int i = topDistZL; i <= bottomDistZL; i++) {
	                                ...
	                                for (int j = leftDistZL; j <= rightDistZL; j++) {
	                                    ...
	                                    if(drawStart-fadingStartTime<CONFIG.FADING_TIME*1E6
	                                    		|| !Util.coveredByTiles(new XYZ(xi,yi,mapLayer.centerXYZ.z), blX, blY, trX, trY, centerXYZ.z, willDrawXYZs)){
	                    ...
	                    for(int ii=0;ii<willDrawTiles.size();ii++){
	                    	...
	                    	float sx=centerScreenX+(willDrawXYs.get(ii).x)*CONFIG.TILE_SIZE;
	                    	float sy=centerScreenY+(willDrawXYs.get(ii).y)*CONFIG.TILE_SIZE;
	                    	mVertexBuffer.clear();
	                    	mVertexBuffer.put(sx-CONFIG.TILE_SIZE/2f+CONFIG.BORDER/2f);
	                    	mVertexBuffer.put(sy-CONFIG.TILE_SIZE/2f+CONFIG.BORDER/2f);
	                    	mVertexBuffer.put(sx-CONFIG.TILE_SIZE/2f+CONFIG.BORDER/2f);
...
//	                    BOOL drawNewText = !zoomingL && ((!_touchBegin && !_beginMoving && !movingL) || movingJustDoneL ||
//                                _isZoom ||
//                                rotatingZJustDoneL ||
//                                rotatingXJustDoneL ||
//                                zoomingJustDoneL);
	                    long start = System.currentTimeMillis();
	                    isWating = (start - lastMoveTime) < 100;
	                    isStaying = touching && !isWating;
	                    isLastLabelFading = isLabelFading;
	    				boolean refreshText = (!zoomingL && !isLabelFading &&
	    						((easingRecord.startMoveTime == 0 && zoomingRecord.digitalZoomEndTime == 0 && !touching)) 
	    						|| zoomingJustDoneL || isManuelZoom || rotatingZJustDoneL || rotatingXJustDoneL
	    						|| (isStaying && !isTouchBegin && !isBeginMoving && !movingL));
	    				if(refreshText) {
	    					LogWrapper.i("Label", "draw all");
	    				}
	    				isLabelFading = this.shownLabels((float)zoomScale, refreshText, true);
	                    ...
	                    if(refreshText) {
	                    	isStaying = false;
	    					LogWrapper.i("Label", "---draw all label cost: " + (end - start));
	    				}
	                    else {
	                    	LogWrapper.i("Label", "+++draw label cost: " + (end - start));
	                    }
	                    gl.glPopMatrix();
	                    gl.glColor4f(1,1,1,1);
...
//	                	gl.glDisable(GL_TEXTURE_2D);
	                	...
	                	int paddingTop = 0;
	                	if (compass.getPlaceLocation().equals(PlaceLocation.CENTER) == false) {
	                	    paddingTop = padding.top;
	                	}
	                	gl.glLoadIdentity();
						gl.glTranslatef(-displaySize.x/2f,displaySize.y/2f+paddingTop, -mapMode.middleZ);
						gl.glRotatef(180,1,0,0);
						
						gl.glTranslatef(displaySize.x/2f,displaySize.y/2f+paddingTop,0);
						float scale=mapMode.middleZ/mapMode.nearZ;
						gl.glScalef(scale, scale, scale);
						gl.glTranslatef(screenXY.x-displaySize.x/2f, screenXY.y-displaySize.y/2f+paddingTop, 0);
						gl.glRotatef(mapMode.getxRotation(), 1, 0, 0);
...
//                    }
    		        ...
    		        if(zoomingL || isLastLabelFading || isWating || isLabelFading || fading || movingL || rotatingX || rotatingZ) {
    		            requestRender();
    		        }
...
//                    MapTextThread.time = 0;
		...
		private void clearTexRefs(GL10 gl){
			LogWrapper.i("TilesView","clean tilePool total texture and image num:"+tileInfos.size());
	        Iterator<TileInfo> iterator1 = tileInfos.values().iterator();
            while(iterator1.hasNext()){
                TileInfo info = iterator1.next();
                if(info.tileTextureRef != 0) {
		...
		private void drawClusterTextOpenGL(GL10 gl, OverlayItem pin, String clusterText, float x, float y){
			...
			float borderSize=ItemizedOverlay.OVERLAY_CLUSTER_BORDER_SIZE*Globals.g_metrics.density;
			float roundRadius=ItemizedOverlay.OVERLAY_CLUSTER_ROUND_RADIUS*Globals.g_metrics.density;
			float textSize=ItemizedOverlay.OVERLAY_CLUSTER_TEXT_SIZE*Globals.g_metrics.density;
			...
			float textOffsetX=ItemizedOverlay.OVERLAY_CLUSTER_TEXT_OFFSET_X*Globals.g_metrics.density;
			float textOffsetY=ItemizedOverlay.OVERLAY_CLUSTER_TEXT_OFFSET_Y*Globals.g_metrics.density;
			...
			if(clusterTextPool.containsKey(key)){
				...
				try{
					...
					RectF rect=new RectF(0,0,totalTextWidth,totalTextHeight);
			...
			x+=clusterOff.x;
			y+=clusterOff.y;
			ItemizedOverlay.OffsetReference relativeTo=overlay.getClusterTextOffsetRelativeTo();
			if(relativeTo.equals(ItemizedOverlay.OffsetReference.BOTTOM_RIGHT)){
				x-=totalTextWidth;
				y-=totalTextHeight;
			}else if(relativeTo.equals(ItemizedOverlay.OffsetReference.BOTTOM_LEFT)){
				y-=totalTextHeight;
			}else if(relativeTo.equals(ItemizedOverlay.OffsetReference.TOP_RIGHT)){
				x-=totalTextWidth;
			}
		...
		private void drawOverlayItemOpenGL(GL10 gl, OverlayItem pin, float x, float y){
			int textureRef=0;
