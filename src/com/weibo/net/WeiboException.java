...
public class WeiboException extends Exception {
	...
	21322	：	OAuth2.0 重定向地址不匹配
	21323	：	OAuth2.0 请求不合法
	21324	：	OAuth2.0 client_id或client_secret参数无效
	21325	：	OAuth2.0 提供的Access Grant是无效的、过期的或已撤销的
	21326	：	OAuth2.0 客户端没有权限
	21327	：	OAuth2.0 token过期
	21328	：	OAuth2.0 不支持的 GrantType
	21329	：	OAuth2.0 不支持的 ResponseType
	21330	：	OAuth2.0 用户或授权服务器拒绝授予数据访问权限
	21331	：	OAuth2.0 服务暂时无法访问
	...
	*/
	...
	public static enum ErrorType {EXPIRED_TOKEN, DUPLICATE_UPLOAD, UPLOAD_FAILED, LOGOUT_FAILED, NETWORK_ERROR, GET_PROFILE_FAILED, OPERATION_FAILED};
	
	public static final int ERROR_CODE_EXPIRED_TOKEN = 21327;
	
	public static final int ERROR_CODE_DUPLICATE_UPLOAD = 20019;
	...
	public void seRequest(String request) {
	...
	public ErrorType getErrorType() {
		...
		case ERROR_CODE_DUPLICATE_UPLOAD:
			return ErrorType.DUPLICATE_UPLOAD;
		default :
			return getUndefinedErrorType();
		}
	}
	
	private ErrorType getUndefinedErrorType() {
		if (Weibo.getInstance().getRequestType() == Weibo.RequestType.UPLOAD) {
			return ErrorType.UPLOAD_FAILED;
		} else if (Weibo.getInstance().getRequestType() == Weibo.RequestType.END_SESSION) {
			return ErrorType.LOGOUT_FAILED;
		} else if (Weibo.getInstance().getRequestType() == Weibo.RequestType.GET_PROFILE) {
			return ErrorType.GET_PROFILE_FAILED;
		}
		return ErrorType.OPERATION_FAILED;
	}
	
	public String toString() {
		return "statusCode: " + statusCode + " error: " + error + " errorCode :" + errorCode + " request :" + request;
	}
